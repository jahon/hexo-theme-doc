...
const PARTIALS = {
  v2: {
    libs: path.resolve(__dirname, './partials/v2/libs.ejs'),
    snippet: path.resolve(__dirname, './partials/v2/snippet.ejs')
  },
  v3: {
    libs: path.resolve(__dirname, './partials/v3/libs.ejs'),
    snippet: path.resolve(__dirname, './partials/v3/snippet.ejs')
  }
};
  ...
  const uiGenerator = (function () {
    ...
    return {
      ...
      getHtml: function ({pageSource, swagger}) {
        ...
        const swaggerUiVersion = hexo.config.theme_config.swagger_ui.version;
        const partials = PARTIALS[`v${swaggerUiVersion}`];
        ...
        const snippetLocals = {
          ...
          swagger,
          url_for,
          swagger_ui_config: hexo.config.theme_config.swagger_ui
        };
        ...
        return hexo
          ...
          .then((html) => {
            return `<div class="hexo-swagger-ui hexo-swagger-ui-v${swaggerUiVersion}">${html}</div>`;
          });
      }
    };
  })();
